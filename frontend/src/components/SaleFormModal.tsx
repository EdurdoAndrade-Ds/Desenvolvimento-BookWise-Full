...
import { salesService, type SaleItemInput } from '../services/salesService';
...
const PAYMENT_METHODS = ['PIX', 'Cartao Credito', 'Cartao Debito', 'Dinheiro'];

export default function SaleFormModal({ onClose, onCreated }: SaleFormModalProps) {
  const [users, setUsers] = useState<User[]>([]);
  const [books, setBooks] = useState<Book[]>([]);
  const [userId, setUserId] = useState<number | ''>('');
  const [paymentMethod, setPaymentMethod] = useState(PAYMENT_METHODS[0]);
  const [items, setItems] = useState<SaleItemInput[]>([{ bookId: 0, quantity: 1 }]);
  ...
  const priceOf = (bookId: number) => books.find((b) => b.id === bookId)?.price ?? 0;
  const total = items.reduce((sum, it) => sum + priceOf(it.bookId) * (it.quantity || 0), 0);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await salesService.create({ userId: Number(userId), paymentMethod, items: validItems });
      onCreated();
      onClose();
    } catch (err) {
      setError(err instanceof ApiError ? err.message : 'Erro ao registrar a venda.');
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-xl bg-white shadow-xl dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 px-6 py-4 dark:border-slate-800">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100">Nova venda</h2>
          <button
            onClick={onClose}
                  ...
                  >
                    ...
                    {books.map((b) => (
                      <option key={b.id} value={b.id}>
                        ...
                        {b.format === 'PHYSICAL' ? ` (estoque: ${b.stock ?? 0})` : ''}
                      </option>
                    ))}
                  </select>
                  <input
